// Einbau ID — same pattern as every other suite app (PUNCH/SCOUT/INTAKE/TALLY/
// HANDOFF). Own login screen, calls auth-worker's /auth/login directly, stores
// the token in localStorage, attaches it as `Authorization: Bearer` on every
// admin call. HELM has no backend of its own — see api.js, which talks to
// auth-worker directly instead of a per-app worker.
